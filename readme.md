...
1、修复了读取配置文件时，因含有某些特殊字符而导致错误的问题

### 2025-12

1、修复了凭证失效重新登录时，无法签到的问题

具体为，网站改版后，签到接口使用未登录时获取的csrf-token时，会导致签到失败

2、修复了在模式2下，程序执行完成后，无法退出的问题

### 2025-08

1、网站改版，被迫修改代码：签到数据不再直接随doc文档返回，而是通过ajax来获取

2、新增了多账号数据共存的特性

切换账号后，新账号是另存一份，不再是覆盖

3、修复了签到金币显示不正确的问题

在此之前，如果通过本脚本之外的方式签到时，会导致"本月累计领取"显示错误

4、修复了windows平台下，修改配置文件后，读取异常的问题

使用记事本修改配置文件后，字符编码由utf-8变为utf-8 with BOM
